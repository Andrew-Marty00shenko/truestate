...
const SliderTwo = ({
    ...
    useEffect(() => {
        setActiveBigPhoto(description[0].image[0])
    }, [])
        ...
        >
            <div className="section-five__block-full-info">
                {description.map(info => {
                    return <div key={info.id}>
                        <p>{info.typeOfRealEstate}: <span> {info.typeOfRealEstateValue}</span></p>
                        <p>{info.location}: <span> {info.locationCountry}</span></p>
                        <p>{info.square}: <span> {info.squareValue}</span></p>
                        <p>{info.price}: <span> {info.priceValue}</span></p>
                        <p>{info.repairAndImprovements}: <span> {info.repairAndImprovementsNubmer}</span></p>
                        <p>{info.fee}: <span> {info.feeNumber}</span></p>
                        <p>{info.priceOfSell}: <span> {info.priceOfSellValue}</span></p>
            ...
            <div className="section-five__block-full-gallery">
                <div className="gallery-top">
                    {description[0].image?.map((i, index) => {
                        return <img
                            src={i}
                            alt=""
                            onClick={() => changeActiveBigPhoto(index)}
                        />
                    })}
                </div>
                <div
                ...
                >
                    <img src={activeBigPhoto} alt="section-five" />
